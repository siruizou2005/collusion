...
├── prompts.py           # 提示词模板常量（固定 P0C）
├── llm_client.py        # OpenAI 兼容 API 调用，含重试与 fallback
...
| 函数 | 说明 |
|------|------|
| `call_llm(prompt, model, ...)` | 调用 OpenAI 兼容接口，含指数退避重试（最多 3 次） |
| `dummy_price_strategy(min, max)` | 无 API Key 时的随机价格 fallback，用于端到端测试 |

**注意**：若 base URL 已含 `/v1`（如阿里云端点），函数会自动避免拼接出 `/v1/v1/chat/completions` 的错误路径。
...
| 常量 | 说明 |
|------|------|
| `PROMPT_P0C` | 唯一提示词：基础角色描述 + 市场波动提醒 |
| `OUTPUT_TEMPLATE` | 规定 LLM 输出格式（观察/计划/洞察/价格） |
...
**`PricingAgent`**：
- `build_prompt(market_history)` — 组装当期完整提示词（前缀 + 产品信息 + 历史 + 记忆 + 格式指令）
- `parse_response(response, floor, ceiling)` — 从 LLM 输出中解析计划、洞察、价格，价格自动限幅到 `[floor, ceiling]`
...
2. 解析失败则启用 `dummy_price_strategy` 随机兜底
...
当噪声不为 `none` 时，自动切换到带 `C` 后缀的提示词（提醒随机波动）。
...
若不配置，系统会自动使用随机价格 fallback，可用于测试非 LLM 部分的流程。
...
系统兼容所有 OpenAI Chat Completions 格式接口。以阿里云 DashScope（国际节点）为例：
...
**示例：** 仅测试高噪声、固定 `P0C` 提示词、α=3.2，运行 5 次
